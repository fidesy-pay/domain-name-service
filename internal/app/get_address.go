package app

import (
	"context"
	"errors"
	"github.com/fidesy-pay/domain-name-service/internal/pkg/redis"
	desc "github.com/fidesy-pay/domain-name-service/pkg/domain-name-service"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
)

func (i *Implementation) GetAddress(ctx context.Context, req *desc.GetAddressRequest) (*desc.GetAddressResponse, error) {
	address, err := i.domainNameService.GetAddress(ctx, req.GetServiceName())
	if err != nil {
		if errors.Is(err, redis.ErrNotFound) {
			return nil, status.Errorf(codes.NotFound, "portResolverService.GetAddress: %v", err)
		}

		return nil, status.Errorf(codes.Internal, "portResolverService.GetAddress: %v", err)
	}

	return &desc.GetAddressResponse{
		Address: address,
	}, nil
}
